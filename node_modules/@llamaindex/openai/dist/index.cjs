...
class OpenAI extends llms.ToolCallLLM {
    ...
    async chat(params) {
        ...
        if (responseFormat && this.metadata.structuredOutput) {
            if (responseFormat instanceof zod.z.ZodType) baseRequestParams.response_format = zod$1.zodResponseFormat(responseFormat, "response_format");
            else {
...
class OpenAIEmbedding extends embeddings.BaseEmbedding {
   ...
   */ this.getTextEmbeddings = async (texts)=>{
        ...
        if (init?.azure || shouldUseAzure()) {
            ...
            this.baseURL = init?.session?.baseURL ?? azureConfig.baseURL ?? env.getEnv("OPENAI_BASE_URL");
